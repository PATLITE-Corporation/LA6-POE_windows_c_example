...
int PNS_RebootCommand(char* password);
...
int PNS_RunControlCommand(PNS_RUN_CONTROL_DATA runControlData)
{
    ...
    // Data area
    memcpy(&sendData[6], &runControlData, sizeof(runControlData));
...
int PNS_DetailRunControlCommand(PNS_DETAIL_RUN_CONTROL_DATA detailRunControlData)
{
    ...
    // Data area
    memcpy(&sendData[6], &detailRunControlData, sizeof(detailRunControlData));
...
int PNS_RebootCommand(char* password)
{
    ...
    // Data size
    sendData[4] = strlen(password) >> 8;
    sendData[5] = (char)(strlen(password) | 0xFF00);

    // Data area
    memcpy(&sendData[6], password, strlen(password));

    // Send PNS command
    ret = SendCommand(sendData, PNS_COMMAND_HEADER_LENGTH + strlen(password), recvData, sizeof(recvData));
    if (ret != 0)
    {
        puts("failed to send data");
        return -1;
...
int PNS_GetDataCommand(PNS_STATUS_DATA* statusData)
{
    ...
    memset(sendData, 0, sizeof(sendData));
    memset(recvData, 0, sizeof(recvData));
    memset(statusData, 0, sizeof(PNS_STATUS_DATA));
    ...
    // Input 1 to 8
    memcpy(statusData->input, &recvData[0], sizeof(statusData->input));
    ...
    {
        // signal light mode
        memcpy(&statusData->ledModeData, &recvData[9], sizeof(statusData->ledModeData));
    }
    else
    {
        // smart mode
        memcpy(&statusData->smartModeData, &recvData[9], sizeof(statusData->smartModeData));
    }
...
int PNS_GetDetailDataCommand(PNS_DETAIL_STATUS_DATA* detailStatusData)
{
    ...
    memset(sendData, 0, sizeof(sendData));
    memset(recvData, 0, sizeof(recvData));
    memset(detailStatusData, 0, sizeof(PNS_DETAIL_STATUS_DATA));
    ...
    // MAC Address
    memcpy(detailStatusData->macAddress, &recvData[0], sizeof(detailStatusData->macAddress));

    // Input 1 to 8
    memcpy(detailStatusData->input, &recvData[6], sizeof(detailStatusData->input));
    ...
    {
        // signal light mode
        memcpy(&detailStatusData->ledModeDetalData, &recvData[19], sizeof(detailStatusData->ledModeDetalData));
    }
    else
    {
        // smart mode
        memcpy(&detailStatusData->smartModeDetalData, &recvData[19], sizeof(detailStatusData->smartModeDetalData));
    }
...
int PHN_WriteCommand(unsigned char runData)
{
    ...
    // check the response data
    if (memcmp(&recvData[0], nak, sizeof(nak)) == 0)
    {
        // receive abnormal response
        puts("negative acknowledge");
        return -1;
